...
const verificationDir = path.join(root, 'verification', 'v2-mainnet')
const deploymentFile = path.join(root, 'deployments', 'v2-mainnet.json')

if (!fs.existsSync(deploymentFile)) throw new Error('Missing deployments/v2-mainnet.json.')

const deployment = JSON.parse(fs.readFileSync(deploymentFile, 'utf8'))
if (Number(deployment.chainId) !== 4663) throw new Error('Deployment record is not Robinhood Chain Mainnet (4663).')

const rpc = process.env.RH_RPC_URL
if (!rpc) throw new Error('RH_RPC_URL is required for mainnet verification.')
if (/testnet/i.test(rpc)) throw new Error('RPC looks like testnet; refusing mainnet verification.')
...
if (actualChainId !== 4663) throw new Error(`RPC chain mismatch; expected 4663, got ${actualChainId}.`)
...
const factoryAddress = deployment.contracts.factoryV2
const feeVaultAddress = deployment.contracts.feeVaultV3
const registryAddress = deployment.contracts.registryV3
...
const contracts = [
  {
    ...
    txHash: deployment.transactions.factoryDeploy,
    args: [owner],
  ...
  },
  {
    ...
    txHash: deployment.transactions.feeVaultDeploy,
  ...
  },
  {
    ...
    txHash: deployment.transactions.registryDeploy,
    args: [factoryAddress],
    argTypes: [{ type: 'address' }],
  },
]
...
console.log('\nRHBurnerPass v2 — MAINNET source verification')
console.log(`Network: Robinhood Chain (${actualChainId})`)
...
async function submit(item) {
  ...
  if (!response.ok) throw new Error(`${item.contractName}: Blockscout submission failed (${response.status}) ${text}`)
...
console.log('\nMainnet verification helper finished.')
